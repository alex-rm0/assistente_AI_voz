...
def test_engine_passes_automatic_contexts_to_llm(tmp_path: Path) -> None:
    ...
    response = engine.respond("Tenho um erro neste projeto Python.")

    assert response == "resposta"
    assert "TECH_CONTEXT" in llm.last_system_prompt
    assert "WORK_CONTEXT" in llm.last_system_prompt
    assert "Contextos ativos" in engine.context_debug()
