...
def test_conversation_registers_and_queries_yesterday(tmp_path: Path) -> None:
    engine = make_engine(tmp_path)

    stored = engine.respond("Ontem estivemos a trabalhar no projeto AssistenteIA com Alexandre.")
    answer = engine.respond("O que fizemos ontem?")

    assert "Registei na timeline" in stored
    assert "AssistenteIA" in answer
    assert "Alexandre" in answer


def test_conversation_answers_project_start(tmp_path: Path) -> None:
    engine = make_engine(tmp_path)

    engine.respond("Ha tres meses comecamos o projeto AssistenteIA.")
    answer = engine.respond("Quando comecamos este projeto?")

    assert "AssistenteIA" in answer
    assert "Comecamos" in answer
